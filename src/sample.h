...
class sample{
 ...
 public:
  ...
  array_2d<num> m_node_map;
  array_3d<num> m_edge_map;
  ...
  num get_edge_potential(int node1, int node2, int state1, int state2){
    if(node1 < node2){
      ...
      return get_edge_potentials(node2, node1, state2, state1);
    }
  }
  ...
  void set_node_potentials(){
	...
	for(int k = 0; k < num_node_features; k++){
	  temp = temp + m_node_features(i,k) * m_node_map(j,k);
  ...
  void set_edge_potentials(){
	...
	for(int k = 0; k < num_states; k++){
	  ...
	  for(int l = 0; l < num_edge_features; l++){
	    temp = temp + m_edge_features(i,l) * m_edge_map(j,k,l);
	  }
  ...
  void set_node_marginals(){
	...
	for(int k = 0; k < num_states; k++){
	  num temp = get_node_potentials(j,k);
  ...
  num get_likelihood(array_1d<int> states){
    num temp = 0;
    for(int i = 0; i < num_nodes; i++){
      temp *= get_node_potentials(i,states(i));
    }
    for(int i = 0; i < num_edges; i++){
      int node1 = pos_to_edge[i].first;
      int node2 = pos_to_edge[i].second;
      temp *= get_edge_potentials(node1, node2, states(node1), states(node2));
    }
    return temp;
  }
